...
public class Game {
 ...
 // 构造函数
    public Game(int maxTurns, Player startPlayer, List<Player> players) {
        this.players = players;  // 使用传入的玩家列表，而不是新创建的空列表
    ...
    public void startGame() {
        ...
        nextTurn();  // 开始第一回合的执行
        scoreManager.calculateRoundScores(); // 开始游戏时计算第一轮分数
    }
    ...
    public void chooseSectorCards() {
        ...
        for (Player player : players) {
            ...
            player.calculatePlayerScore(chosenCard, hexBoard);  // Pass the chosen card to calculate score
        }
    }
    ...
    public static void main(String[] args) {
        ...
        // 创建一个临时的 Game 对象，用于传递给 Player 的构造函数
        Game tempGame = new Game(9, null, players);  // 初始时没有 startPlayer，暂时传 null
